...
class Pipeline(object):
    def __init__( self, steps, base_dir, logger=None, verbosity=1, log_file=None, name="pipeline", config=None,
        ...
        self.logger.error("Pipeline set up to run the following steps: %s"%' '.join([s.name for s in self.steps]))
    ...
    def from_config_file(cls, config_file, base_dir, logger=None, verbosity=1,
        ...
        if "template" in config:
            ...
            if not os.path.isabs(config["template"]):
                if (cwd != os.path.dirname) and (os.path.isfile(os.path.join(config_dirname, config["template"]))):
                    #we're running from a different directory to where the main config file is, and a config file with the name
                    ...
                    #name in cwd
                    template_file_to_use = os.path.join(config_dirname, config["template"])
                    if os.path.isfile(config["template"]):
                        logger.error("Using template config %s, not %s. Make sure this is what you want"%(template_file_to_use, os.path.join(cwd, config["template"])))
        ...
        if step_names is None:
            step_names = config["pipeline"]["steps"] #(config["steps"].strip()).split()
        ...
        for step_name in step_names:
            if step_name == "galsim":
                ...
                try:
                    ...
                    logger.error("no entry for step %s found in config file, continuing with empty step_config"%step_name)
                    step_config = {}
                ...
                if "verbosity" in step_config:
                    ...
                    step_verbosity=1
                steps.append( step_class(step_config, base_dir, logger=logger, verbosity=step_verbosity, name=step_name) )
    ...
    def execute(self, new_params_list=None, base_dir=None, no_overwrite_job_record=False,
        ...
        for step,new_params in zip(self.steps, new_params_list):
            ...
            if status != 0:
                self.logger.error(PANIC_STRING+"step %s return status %d: quitting pipeline here\n"%(step.name, status)+PANIC_STRING)
                return status
            else:
    ...
    def _save_restart(self, no_overwrite_job_record):
        if no_overwrite_job_record is False:
            #Save stash
            self.logger.error("Saving job record with completed steps %s"%(str(self.stash["completed_step_names"])))
    ...
    def set_step_config(self, step_name, key, value):
        ...
        while n_nest < len(key_split)+1:
            ...
            config = d[k]
            n_nest += 1
        self.logger.error("Setting key %s in step %s to: %s"%(key, step_name, str(value)))
        d[k] = value
