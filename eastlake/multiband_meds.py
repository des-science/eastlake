...
class MultibandMEDSBuilder(MEDSBuilder):
    ...
    def setup(self, config, base, file_num, logger):
        ...
        if 'tile_num' not in galsim.config.process.valid_index_keys:
            ...
            galsim.config.eval_base_variables += ['tile_num', 'band_num', 'tile_start_obj_num', 'band', "tilename", "nbands"]
        ...
        if not isinstance(nobj, int):
            raise ValueError("nobjects is required to be a fixed integer for type 'MultibandMEDS'")
        ...
        if not isinstance(rs,list):
            ...
            try:
                ...
                print("image.random_seed must be set to a non-zero integer for output type 'MultibandMEDS'")
                raise(e)
        ...
        nepochs_per_object = galsim.config.ParseValue(config, 'nstamps_per_object', base, int)[0]

        #tile_start_obj_num is the object number of the first object in the current tile
        #for multiband-meds, this is the following.
        base['tile_start_obj_num'] = base['start_obj_num'] - band_num * nobjects * nepochs_per_object
    ...
    def buildImages(self, config, base, file_num, image_num, obj_num, ignore, logger):
        ...
        logger.debug("ntiles, nbands, tile_num, band_num = %d, %d, %d, %d",ntiles,nbands,tile_num,band_num)

        # The rest is pretty much the same as the base class (galsim.des.des_meds.MEDSBuilder), except for how the 'id' field is set.
        ...
        if psf_s2n is not None:
            ...
            for i, psf in enumerate(psf_images):
                ...
                orig_psf_vals = psf_images[i].array.copy()
                noise_var = psf_images[i].addNoiseSNR(noise, psf_s2n, preserve_flux=True)
