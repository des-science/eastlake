...
class SmoothPiffLoader(galsim.config.InputLoader):
...
galsim.config.RegisterInputType('des_smoothpiff', SmoothPiffLoader(DES_SmoothPiff))


# and a builder
def BuildDES_SmoothPiff(config, base, ignore, gsparams, logger):
    des_piff = galsim.config.GetInputObj('des_smoothpiff', config, base, 'DES_SmoothPiff')
    ...
    if 'wcs' not in base:
        raise galsim.GalSimConfigError(
            "DES_SmoothPiff requested, but no wcs defined in base.")
    wcs = base['wcs']
...
def BuildDES_SmoothPiff_with_substitute(config, base, ignore, gsparams, logger):
    # This builder usually just calls BuildDES_SmoothPiff, but can also
    ...
    if use_substitute:
        ...
        return BuildDES_SmoothPiff(config, base, ignore, gsparams, logger)


galsim.config.RegisterObjectType(
    'DES_SmoothPiff', BuildDES_SmoothPiff_with_substitute, input_type='des_smoothpiff')
